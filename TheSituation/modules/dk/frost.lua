...
function mod.OnInitialize()
	db = PA:RegisterClassModuleDB(modName, defaults)
end
...
function mod.rotation(StartTime, GCDTimeLeft)
	if (enabled) then
		...
		DeathAndDecayCD, DeathAndDecayFinish		= PA:GetActualSpellCD(DeathAndDecayFinish, "Death and Decay")
		...
		elseif(KM and ob) then
			Spell1 = Obliterate
			PA:SetTitle("Oblit KM")
		elseif(readyUF >= 2) then
			Spell1 = Obliterate
			PA:SetTitle("Oblit 2xUF")
		elseif(readyRunes[4] >= 2) then
			Spell1 = Obliterate
			PA:SetTitle("Oblit 2xDR")
...
function mod.OnInitialize()
	mod.checkSpec()
	if (enabled) then
		PA:Print("Initializing Frost")
		PA:SetToggle(1, 0, "Death Coil")		
		PA:SetToggle(2, 0, "Necrotic Strike")
		-- PA:SetToggle(3, 1, nil)
		...
		Obliterate				= PA:RegisterSpell(9, "Obliterate")
		FrostStrike				= PA:RegisterSpell(10, "Frost Strike")
		HowlingBlast			= PA:RegisterSpell(11, "Howling Blast")
		MindFreeze				= PA:RegisterSpell(12, "Mind Freeze")
